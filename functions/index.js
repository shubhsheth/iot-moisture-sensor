...
app.onSync((body) => {
	...
	for (devicecounter = 0; devicecounter < deviceitems.length; devicecounter++) {
		...
		if (deviceitems[devicecounter].traits.includes('action.devices.traits.SensorState')) {
			firebaseRef.child(deviceitems[devicecounter].id).child('SensorState').set({ currentSensorStateData: [{ "name": "FilterLifeTime", "rawValue": 10 }] });
		}
	}
	return {
...
exports.reportstate = functions.database.ref('{deviceId}').onWrite(async (change, context) => {
	...
	if (Object.prototype.hasOwnProperty.call(snapshot, 'SensorState')) {
		if (Object.prototype.hasOwnProperty.call(snapshot.SensorState, 'currentSensorStateData')) {
			syncvalue = Object.assign(syncvalue, { currentSensorStateData: snapshot.SensorState.currentSensorStateData });
		}
	}
